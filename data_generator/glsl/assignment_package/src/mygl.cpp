...
void MyGL::initializeGL()
{
    ...
    m_glintNoiseFB.addTexture(GBufferOutputType::GLINT_NOISE);
...
void MyGL::paintGL() {
    ...
    m_glintNoiseFB.bindToTextureSlot(GLINT_NOISE_TEX_SLOT, GBufferOutputType::GLINT_NOISE);
    m_progPBR.setUnifInt("u_GlintNoiseTexture", GLINT_NOISE_TEX_SLOT);
    m_progPBR.setUnifInt("_Glint2023NoiseMapSize", m_glintNoiseFB.width());
...
void MyGL::renderGlintNoiseToTexture(unsigned int size, unsigned int seed) {
    ...
    // fragment shader
    m_progGlintNoise.setUnifInt("_FrameSize", size);
    m_progGlintNoise.setUnifInt("_Seed", seed);
    ...
    // The glint noise texture is added to m_outputTextures with addTexture()
    Texture &t = *(m_glintNoiseFB.getOutputTextures().at(GBufferOutputType::GLINT_NOISE));
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, t.getHandle(), 0);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
