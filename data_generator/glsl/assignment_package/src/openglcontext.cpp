...
void OpenGLContext::debugContextVersion()
{
    ...
    if (ctx->hasExtension("GL_ARB_shading_language_packing")) {
        qDebug() << "GL_ARB_shading_language_packing is supported!";
    } else {
        qDebug() << "GL_ARB_shading_language_packing is NOT supported!";
    }
}
