...
def validate_itr(glctx, target, geometry, opt_material, lgt, FLAGS):
    ...
    with torch.no_grad():
        ...
        buffers = geometry.render(glctx, target, lgt, opt_material, FLAGS=FLAGS)
        ...
        if FLAGS.display is not None:
            ...
            for layer in FLAGS.display:
                if "latlong" in layer and layer["latlong"]:
                    ...
                    img = geometry.render(
                        glctx, target, layer["relight"], opt_material, FLAGS=FLAGS
                    )
...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it):
        ...
        return self.geometry.tick(
            glctx, target, self.light, self.material, self.image_loss_fn, it, FLAGS
        )
...
def optimize_mesh(
    ...
    if FLAGS.multi_gpu:
        ...
        if optimize_geometry:
            optimizer_mesh = apex.optimizers.FusedAdam(
                trainer_noddp.geo_params, lr=learning_rate_pos, betas=betas
            )
    ...
    if save_models:
        torch.save(geometry, os.path.join(FLAGS.out_dir, f"geometry_{pass_name}.pt"))
        torch.save(
            opt_material, os.path.join(FLAGS.out_dir, f"material_{pass_name}.pt")
        )
        torch.save(lgt, os.path.join(FLAGS.out_dir, f"light_{pass_name}.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--isosurface", default="dmtet", choices=["dmtet", "flexicubes"]
    )
    ...
    env_map_lst = ["clarens", "road", "golf"] if obj_name != 'sphere' else ["e10"]
    env_map = env_map_lst[0]

    geometry = torch.load(f"./models/{obj_name}/geometry_{env_map}.pt")
    lgt = torch.load(f"./models/{obj_name}/light_{env_map}.pt")
    mat = torch.load(f"./models/{obj_name}/material_{env_map}.pt")

    mat["glint_params"] = get_stats_of_obj(
        f"./data/nerf_synthetic/{obj_name}/{env_map}"
    )
    ...
    if FLAGS.validate and FLAGS.local_rank == 0:
        validate(
            ...
            lgt,
            dataset_validate,
            os.path.join(FLAGS.out_dir, "glint_validate"),
            FLAGS,
        )

    if FLAGS.local_rank == 0:
        final_mesh = geometry.getMesh(mat)
        os.makedirs(os.path.join(FLAGS.out_dir, "glint_mesh"), exist_ok=True)
        obj.write_obj(os.path.join(FLAGS.out_dir, "glint_mesh/"), final_mesh)
        light.save_env_map(os.path.join(FLAGS.out_dir, "glint_mesh/probe.hdr"), lgt)
