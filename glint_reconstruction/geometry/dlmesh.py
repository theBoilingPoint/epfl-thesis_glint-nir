...
from render import mesh, render, regularizer, util
...
class DLMesh(torch.nn.Module):
    ...
    def render(self, glctx, target, lgt, opt_material, bsdf=None, FLAGS=None):
        opt_mesh = self.getMesh(opt_material)
        return render.render_mesh(glctx, opt_mesh, target['mvp'], target['campos'], lgt, target['resolution'], spp=target['spp'], 
                                    num_layers=self.FLAGS.layers, msaa=True, background=target['background'], bsdf=bsdf, FLAGS=FLAGS)

    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration, FLAGS):
        # ==============================================================================================
        #  Render optimizable object with identical conditions
        # ==============================================================================================
        buffers = self.render(glctx, target, lgt, opt_material, FLAGS=FLAGS)
        ...
        color_img = torch.clamp(buffers['shaded'], 0.0, 1.0)

        masked_ref = color_ref[..., 0:3] * color_ref[..., 3:]
        masked_img = color_img[..., 0:3] * color_ref[..., 3:]

        # Initialise losses
        img_loss = loss_fn(masked_img, masked_ref)
